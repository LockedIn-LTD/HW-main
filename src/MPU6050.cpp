...
MPU6050::MPU6050(int8_t addr, const char* bus)
      ...
      G_OFF_X(0), G_OFF_Y(0), G_OFF_Z(0)
{
    ...
    // Wake up MPU6050
    i2c_smbus_write_byte_data(f_dev, 0x6B, 0x00);
    i2c_smbus_write_byte_data(f_dev, 0x1B, GYRO_CONFIG);
    i2c_smbus_write_byte_data(f_dev, 0x1C, ACCEL_CONFIG);
...
void MPU6050::_readRaw() {
    ...
    az = (float)(raw_az - A_OFF_Z) / ACCEL_SENS;

    gr = (float)(raw_gr - G_OFF_X) / GYRO_SENS;
    gp = (float)(raw_gp - G_OFF_Y) / GYRO_SENS;
    gy = (float)(raw_gy - G_OFF_Z) / GYRO_SENS;
}
...
void MPU6050::_update() {
    ...
    _accel_angle[0] = atan2(ay, az) * RAD_TO_DEG;
    _accel_angle[1] = atan2(-ax, sqrt(ay * ay + az * az)) * RAD_TO_DEG;
    if (calc_yaw)
...
void MPU6050::calibrate(int samples) {
    ...
    A_OFF_X = ax_sum / samples;
    A_OFF_Y = ay_sum / samples;
    A_OFF_Z = az_sum / samples - ACCEL_SENS; // subtract 1g
    G_OFF_X = gr_sum / samples;
    G_OFF_Y = gp_sum / samples;
    G_OFF_Z = gy_sum / samples;
